...
/// ViewModel for the artists list with pagination and pull-to-refresh.
@Observable
final class ArtistsViewModel {
    ...
    func loadArtists() async {
        ...
        do {
            let response: PaginatedResponse<Artist> = try await apiClient.request(
                endpoint: "/api/v1/artists?limit=\(pageSize)&offset=0"
            )
    ...
    func loadMore() async {
        ...
        do {
            let response: PaginatedResponse<Artist> = try await apiClient.request(
                endpoint: "/api/v1/artists?limit=\(pageSize)&offset=\(currentOffset)"
            )
